...
export const getTrending = async (): Promise<Api.TV[]> => {
    const { results } = await db<Api.Page<Api.TV>>("/trending/all/day?language=en-US");
    return results;
};

export const getShowById = async (id: number): Promise<Api.TVDetails> => {
    return db<Api.TVDetails>(`/tv/${id}?language=en`);
};

export const getShowById2 = async (id: number): Promise<Api.TVDetails> => {
    return db<Api.TVDetails>(`/movie/${id}?language=en`);
};
...
export const getGenres = async (): Promise<Api.Genre[]> => {
    const { genres } = await db<Api.GenreList>(`/genre/movie/list?language=en`);
    return genres;
};

export const getShowsByGenre = async (genre: Api.Genre): Promise<Record<string, Api.TV[]>> => {
    const {results} = await db<Api.Page<Api.TV>>("/discover/movie?language=en", `&with_genres=${genre.id}`);
